...
class SpotlightPreferences @Inject constructor(
    ...
    val loggedInFlow: Flow<Boolean> = userPreferences.data.map { preferences ->
        preferences[PreferencesKeys.loggedIn] ?: true
    }

    val accessTokenFlow: Flow<String> = userPreferences.data.map { preferences ->
        preferences[PreferencesKeys.accessToken] ?: ""
    }

    val expiresAtFlow: Flow<String?> = userPreferences.data.map { preferences ->
